...
def ping(host: str, count: int = 4) -> list[float]:
    ...
    if p.returncode != 0:
        return []
    import re
    vals = re.findall(r"(?:time|time=)[=<]\s*([0-9]+(?:\.[0-9]+)?)", p.stdout.lower())
...
async def diagnose(_: Empty | None = None):
    ...
    checks['Internet']={'status':'ok' if samples else 'bad','value':f'{len(samples)}/{4} replies' if samples else 'No replies','detail':'ICMP reachability to a public resolver'}
    if samples:
        ...
        checks['Stability']={'status':'ok' if len(samples)==4 else 'warn','value':f'{100*len(samples)/4:.0f}% replies','detail':'Packet-loss proxy from 4 ICMP probes'}
    else:
